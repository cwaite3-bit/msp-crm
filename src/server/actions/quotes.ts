...
// generic digest (shown to the user as "Minified React error #441"), so
// this quote's two validation errors need to come back as data for
// quote-actions.tsx to actually display them. Same pattern as
// pushQuoteToQuickBooks / setQuoteStatus.
export async function resetQuote(quoteId: string): Promise<{ ok: boolean; error?: string }> {
  try {
    await requireUser();
    const [quote] = await db.select().from(quotes).where(eq(quotes.id, quoteId)).limit(1);
    ...
    if (quote.quickbooksInvoiceId) {
      return {
        ok: false,
        error: "This quote has already been invoiced in QuickBooks — resetting it here would orphan that invoice. Create a new quote instead.",
      };
    }
    ...
    return { ok: false, error: err instanceof Error ? err.message : "Could not reset this quote" };
  }
}
